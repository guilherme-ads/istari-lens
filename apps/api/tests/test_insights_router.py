...
def test_integrations_list_returns_all_items() -> None:
    ...
    try:
        admin = session.query(User).filter(User.email == "insights@test.com").first()
    ...
    with client:
        response = client.get("/insights/integrations")
    assert response.status_code == 200, response.text
    payload = response.json()
...
def test_activate_integration_deactivates_previous_active() -> None:
    ...
    with client:
        response = client.patch(f"/insights/integrations/{second_id}/activate")
    assert response.status_code == 200, response.text
    payload = response.json()
...
def test_refresh_billing_persists_snapshot_and_exposes_on_list() -> None:
    client, _session_factory = _create_app(with_integration=True)
    original_fetch_costs = insights._fetch_openai_costs

    async def _fake_fetch_costs(*, api_key: str, start_time, end_time) -> float:
        _ = api_key, start_time, end_time
        return 12.34

    insights._fetch_openai_costs = _fake_fetch_costs
    try:
        with client:
            refresh = client.post("/insights/integrations/billing/refresh")
            listed = client.get("/insights/integrations")
    finally:
        insights._fetch_openai_costs = original_fetch_costs
...
def test_extract_billing_total_reads_cost_buckets() -> None:
    ...
    assert insights._extract_billing_total_usd(payload) == 4.0


def test_fetch_openai_costs_uses_pagination() -> None:
    original_client = insights.httpx.AsyncClient
    calls: list[dict] = []
    ...
    insights.httpx.AsyncClient = _FakeAsyncClient
    try:
        total = asyncio.run(
            insights._fetch_openai_costs(
                api_key="sk-test",
        ...
        insights.httpx.AsyncClient = original_client
...
def test_fetch_openai_costs_raises_clear_error_on_permission_denied() -> None:
    ...
    try:
        ...
        except insights.HTTPException as exc:
