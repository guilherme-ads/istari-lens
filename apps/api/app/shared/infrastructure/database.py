...
engine = create_engine(
    settings.app_db_url, poolclass=QueuePool, pool_size=10, max_overflow=20, echo=False
)
...
async def get_analytics_connection():
    ...
    try:
        ...
        raise OperationalError("Failed to connect to analytics database") from e
    except Exception as e:
        raise Exception(
            "An unexpected error occurred while connecting to analytics database"
        ) from e
